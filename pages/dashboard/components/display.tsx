...
export default function DisplayComponentPage() {
  ...
  const handleSortChange = (col: string) => {
    ...
    else { setSortTab(col); setSortReverse(false); }
  };

  const handleProtocolSortChange = (col: string) => {
    if (col === protocolSortTab) setProtocolSortReverse(r => !r);
    else { setProtocolSortTab(col); setProtocolSortReverse(false); }
  };
    ...
    <>
      ...
      <div className="px-6 py-8 max-w-screen-xl">
        ...
        <Section title="Badge">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <Card>
              ...
              <div className="flex flex-wrap gap-3">
                <Badge text="Active" variant="custom" customColor="text-brand" customBgColor="bg-brand/20" />
                <Badge text="Pending" variant="custom" customColor="text-info" customBgColor="bg-info/20" />
                <Badge text="Paused" variant="custom" customColor="text-purple-400" customBgColor="bg-purple-400/20" />
                <Badge text="Closed" variant="custom" customColor="text-text-muted" customBgColor="bg-surface" />
              </div>
            </Card>
            ...
            <Card>
              ...
              <div className="space-y-3 text-sm">
                {[
                  ...
                  { label: 'Frankencoin', chain: 'Ethereum', status: 'Pending', risk: 'medium' as const },
                  ...
                  <div key={row.label} className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      <Badge text={row.chain} variant="custom" customColor="text-info" customBgColor="bg-info/10" size="sm" />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <Card>
              ...
              <div className="flex flex-wrap gap-4">
                <IconLogo icon={<FontAwesomeIcon icon={faWallet} className="text-brand" />} size={10} />
                <IconLogo icon={<FontAwesomeIcon icon={faChartLine} className="text-success" />} size={10} />
                <IconLogo icon={<FontAwesomeIcon icon={faShield} className="text-info" />} size={10} />
                <IconLogo icon={<FontAwesomeIcon icon={faArrowRightArrowLeft} className="text-purple-400" />} size={10} />
                <IconLogo icon={<FontAwesomeIcon icon={faLightbulb} className="text-yellow-400" />} size={10} />
              </div>
            </Card>
          </div>
        </Section>
          ...
          <Card>
            ...
            <div className="flex flex-wrap gap-3">
              ...
              <ButtonInput
                label="Error"
                variant="error"
                onClick={() => showToast.error('Transaction failed. Insufficient balance for gas fees.')}
              />
              <ButtonInput
                label="Info"
                variant="secondary"
        ...
        <Section title="Modal Components">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <Card>
              ...
              <ButtonInput label="Open modal" variant="primary" onClick={() => setBasicModalOpen(true)} />
            </Card>

            <Card>
              <CardTitle title="With footer" />
              <ButtonInput label="Open modal with footer" variant="secondary" onClick={() => setFooterModalOpen(true)} />
            </Card>

            <Card>
              <CardTitle title="ConfirmModal" />
              <ButtonInput label="Confirm action" variant="outline" onClick={() => setConfirmModalOpen(true)} />
            </Card>
            ...
            <TableBody>
              {filteredPositions.length === 0 ? (
                ...
                filteredPositions.map(pos => (
                  <TableRow key={pos.asset} headers={POSITION_HEADERS} tab={sortTab} colSpan={4} rawHeader>
          ...
          <Table>
            ...
            <TableBody>
              {PROTOCOL_ROWS.map(row => (
                <TableRow key={row.protocol} headers={PROTOCOL_HEADERS} tab={protocolSortTab} colSpan={4} rawHeader>
                  <div className="text-left font-semibold text-text-primary flex items-center gap-2">
        ...
        <Section title="HeroSteps">
          <div className="space-y-6">
            <div>
              ...
              <HeroSteps
                steps={[
                  { icon: '1', title: 'Connect wallet', description: 'Link MetaMask, WalletConnect, or Coinbase to get started.' },
                  { icon: '2', title: 'Select a vault', description: 'Browse lending vaults and pick one that fits your risk profile.' },
                  { icon: '3', title: 'Deposit & earn', description: 'Supply assets to start earning yield. Withdraw at any time.' },
                ]}
              />
            </div>
            <div>
              ...
              <HeroSteps
                steps={[
                  { icon: '1', title: 'Approve token', description: 'Grant the protocol permission to move your tokens. One-time transaction.' },
                  { icon: '2', title: 'Confirm deposit', description: 'Sign the deposit transaction to lock collateral and mint ZCHF.' },
                ]}
              />
            </div>
          </div>
        ...
        <Section title="Breadcrumb">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <Card>
              ...
              <Breadcrumb items={[{ label: 'Dashboard', href: '/dashboard' }, { label: 'Vaults', href: '/vaults' }, { label: 'Morpho Blue' }]} />
            </Card>
            ...
            <Card>
              ...
              <Breadcrumb items={[{ label: 'Overview', onClick: () => {} }, { label: 'Strategies', onClick: () => {} }, { label: 'BTC Covered Call' }]} />
            </Card>
        ...
        <Section title="Section Component">
          <Section title="Default variant" description="Children render directly with no background wrapper.">
          ...
          <Section title="Filled variant" description="Subtle background, no card border." variant="filled">
            <p className="text-text-secondary text-sm">
      ...
      {/* ── Portaled modals ─────────────────────────────────────────────── */}
      <Modal isOpen={basicModalOpen} onClose={() => setBasicModalOpen(false)} title="Vault Details" size="md">
      ...
      <Modal
        ...
        footer={
          <ButtonInput
            label="Confirm"
            variant="primary"
            onClick={() => { setFooterModalOpen(false); showToast.success('Deposit submitted successfully.'); }}
            second={{ label: 'Cancel', variant: 'secondary', onClick: () => setFooterModalOpen(false) }}
          />
        }
      >
        <div className="space-y-4 text-sm text-text-secondary">
          <p>
            ...
            <span className="text-text-primary font-semibold">3.8 ETH</span> into the Morpho Blue vault.
          </p>
      ...
      <ConfirmModal
        ...
        onConfirm={() => { setConfirmModalOpen(false); showToast.success('Withdrawal submitted.'); }}
      />
      ...
      <ConfirmModal
        ...
        message={
          <span>
            This will <span className="text-error font-semibold">permanently close</span> your position and return collateral to your wallet. Proceed?
          </span>
        }
        confirmText="Delete"
        cancelText="Cancel"
        confirmVariant="danger"
        onConfirm={() => { setDangerModalOpen(false); showToast.error('Position deleted.'); }}
      />
    </>
  );
}
