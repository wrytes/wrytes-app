...
export default function DeribitPositionsPage() {
  ...
  const positionsBtc = useDeribitFetch<Position[]>('/trading/positions?currency=BTC');
  const positionsEth = useDeribitFetch<Position[]>('/trading/positions?currency=ETH');
  ...
  const positions = currency === 'BTC' ? positionsBtc : positionsEth;
  const orders = currency === 'BTC' ? ordersBtc : ordersEth;

  const refetchAll = () => {
    summaryBtc.refetch();
    summaryEth.refetch();
    positionsBtc.refetch();
    positionsEth.refetch();
    ordersBtc.refetch();
    ordersEth.refetch();
  };
      ...
      <Section>
        ...
        <div className="mt-8">
          ...
          {positions.loading ? (
            ...
            <AgentError message={positions.error} onRetry={positions.refetch} />
          ) : !positions.data?.length ? (
            <Card>
              <p className="text-text-muted text-sm text-center py-4">No open positions.</p>
            </Card>
          ) : (
              ...
              <table className="w-full text-sm min-w-[800px]">
                <thead>
                  <tr className="border-b border-surface">
                    {['Instrument', 'Kind', 'Side', 'Size', 'Avg Price', 'Mark Price', 'Float P&L', 'Total P&L', 'Delta'].map(h => (
                ...
                <tbody>
                  {positions.data.map((p, i) => (
                    <tr key={p.instrument_name} className={i % 2 === 0 ? '' : 'bg-surface/30'}>
                      <td className="px-4 py-3 text-text-primary font-mono text-xs">{p.instrument_name}</td>
                      <td className="px-4 py-3 text-text-secondary text-xs">{p.kind}</td>
                      <td className={`px-4 py-3 font-medium text-xs ${p.direction === 'buy' ? 'text-success' : p.direction === 'sell' ? 'text-error' : 'text-text-muted'}`}>
                        {p.direction.toUpperCase()}
                      </td>
                      <td className="px-4 py-3 text-text-primary font-mono">{p.size}</td>
                      <td className="px-4 py-3 text-text-primary font-mono">{fmt(p.average_price, 4)}</td>
                      <td className="px-4 py-3 text-text-primary font-mono">{fmt(p.mark_price, 4)}</td>
                      <td className={`px-4 py-3 font-mono ${p.floating_profit_loss >= 0 ? 'text-success' : 'text-error'}`}>
                        {fmt(p.floating_profit_loss, 4)}
                      </td>
                      <td className={`px-4 py-3 font-mono ${p.total_profit_loss >= 0 ? 'text-success' : 'text-error'}`}>
                        {fmt(p.total_profit_loss, 4)}
                      </td>
                      <td className="px-4 py-3 text-text-secondary font-mono">{fmt(p.delta, 3)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
