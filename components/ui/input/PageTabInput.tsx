...
export default function PageTabInput({ tabs, className }: Props) {
  ...
  return (
    <div className={className}>
      <div className="flex gap-6 border-b-2 border-surface">
        {tabs.map((tab, i) => (
          <button
            key={i}
            onClick={() => setActive(i)}
          ...
          >
            ...
            {tab.badge != null && tab.badge > 0 && (
              <span className="inline-flex items-center justify-center w-5 h-5 rounded-full bg-error text-white text-xs font-bold leading-none">
                {tab.badge}
              </span>
            )}
          </button>
